...
if (isset($_POST['submit'])){
    if(!empty($_POST['Title']) AND !empty($_POST['Description'])AND !empty($_POST['Date']) AND !empty($_POST['nbPlaces'])){
        ...
        $req_Add_event = $mysql->prepare('INSERT INTO evenement(titre,description,dateEvent,nbrInscrit,nbPlaceMax) VALUES(?, ?, ?, ?, ?)');
        $req_Add_event->execute(array($event_title, $event_desc, $event_date,0,$event_nbPlaces));
